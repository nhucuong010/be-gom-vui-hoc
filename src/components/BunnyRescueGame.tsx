...
const SCENES = {
    river: {
        ...
        gom: 'br_gom_jump.png',
        bunnyWait: 'br_bunny_wait.png',
        bunnyHappy: 'br_bunny_happy.png',
...
const BunnyRescueGame: React.FC<BunnyRescueGameProps> = ({ onGoHome, onCorrectAnswer, isSoundOn }) => {
    ...
    useEffect(() => {
        if (phase === 'operation_select') {
             setCurrentScene(Math.random() > 0.5 ? SCENES.river : SCENES.forest);
        }
    }, [phase]);
    ...
    const startLevel = useCallback((opType: OperationType) => {
        ...
        setPhase('playing');
        playDynamicSentence("Bạn Thỏ Bông đang đợi ở cuối đường. Gốm ơi cứu tớ với!", 'vi', isSoundOn, 'bunny_rescue');
    }, [isSoundOn]);

    const handleAnswer = (answer: number) => {
        if (feedback || isHopping || !currentLevelData) return;
        ...
        if (correct) {
            ...
            playDynamicSentence("Hay quá! Nhảy một bước nào.", 'vi', isSoundOn, 'bunny_rescue');

            // 1. Vật phẩm (Lá sen/Nấm) nổi lên
            ...
            setTimeout(() => {
                setIsHopping(true);
                playSound('jump', isSoundOn); // Play jump sound
                ...
                setTimeout(() => {
                    ...
                    if (currentStepIndex >= 4) {
                        ...
                        const winSequence = async () => {
                            await playDynamicSentence("Tuyệt vời! Gốm đã cứu được Thỏ rồi.", 'vi', isSoundOn, 'bunny_rescue');
            ...
            playDynamicSentence("Chưa đúng rồi, bạn Thỏ đang đợi, thử lại nhé.", 'vi', isSoundOn, 'bunny_rescue');
            setTimeout(() => {
                setPhase('help');
                setFeedback(null);
            }, 2000);
            ...
            <div className="w-full max-w-5xl mx-auto flex flex-col items-center p-4 min-h-screen justify-center">
                <div className="w-full bg-white/90 backdrop-blur-md rounded-[3rem] shadow-2xl p-8 md:p-12 relative overflow-hidden border-4 border-purple-100">
                    ...
                    <div className="text-center mb-12">
                        ...
                        <p className="text-2xl text-pink-500 font-bold">Gốm ơi, hãy giải toán để tạo lối đi cứu bạn Thỏ nhé!</p>
                    </div>
        ...
        <div className="w-full max-w-7xl mx-auto flex flex-col items-center p-2 md:p-6 min-h-screen justify-start">
            <div className="w-full bg-white/90 backdrop-blur-sm rounded-[2.5rem] shadow-2xl p-4 md:p-6 relative overflow-hidden flex flex-col gap-6">
                ...
                <div className="relative w-full aspect-[16/9] md:aspect-[21/9] bg-cover bg-center rounded-3xl shadow-inner overflow-hidden border-4 border-white" 
                    ...
                    {/* Đích đến (Bạn Thỏ) - Ẩn khi thắng để hiện hình ôm nhau */}
                    <div className="absolute transition-all duration-500 z-10" style={{ top: `${END_POS.top}%`, left: `${END_POS.left}%`, transform: 'translate(-50%, -50%)' }}>
                        {!isWin && (
                            <div className="relative">
                                ...
                                <img 
                                    src={`${ASSET_BASE_URL}/${currentScene.bunnyWait}`} 
                                    alt="Bunny" 
                    ...
                    >
                        <div className={`relative ${isHopping ? 'animate-jump-arc' : ''}`}>
                             {phase === 'help' && <SpeechBubble text="Khó quá..." position="left" />}
                             {isWin && <SpeechBubble text="Yeahhh!" position="left" />}
                             <img 
                                src={`${ASSET_BASE_URL}/${isWin ? 'br_gom_hug_bunny.png' : (phase === 'help' ? 'br_gom_thinking.png' : currentScene.gom)}`} 
                                alt="Gốm" 
                ...
                {isWin ? (
                    <div className="text-center py-4 animate-fade-in">
                        ...
                        <p className="text-2xl text-gray-600 mb-6">Gốm đã đến nơi cứu bạn Thỏ!</p>
                    ...
                    currentProblem && (
                        <div className="flex flex-col items-center justify-center gap-6 pb-4">
                            {/* Question */}
                            <div className="relative">
                                <div className="absolute -inset-4 bg-gradient-to-r from-blue-200 to-teal-200 rounded-full blur opacity-50"></div>
