...
const EnglishGame: React.FC<EnglishGameProps> = ({ onGoHome, onCorrectAnswer, isSoundOn }) => {
        ...
        return (
            <div className="w-full max-w-6xl mx-auto">
                <div className="w-full bg-white bg-opacity-80 backdrop-blur-sm rounded-3xl shadow-xl p-8 relative">
                    ...
                    <h2 className="text-6xl md:text-7xl font-bold text-center text-purple-700 mb-12 mt-12">Bé Học Tiếng Anh</h2>
                           ...
                           <button 
                            ...
                            >
                                ...
                                <div className="text-center">
                                    <p className="text-2xl font-semibold text-pink-500">Bài {lesson.lesson}</p>
                                    <h3 className="text-4xl font-bold mt-1 leading-tight">{lesson.title}</h3>
                                </div>
                            </button>
                        ))}
                    </div>
    ...
    return (
        <div className="w-full max-w-2xl mx-auto">
            <div className="w-full bg-white bg-opacity-80 backdrop-blur-sm rounded-3xl shadow-xl p-8 relative">
                 <button onClick={() => { playSound('click', isSoundOn); stopAllSounds(); setSelectedLesson(null); }} className="absolute top-4 left-4 text-purple-500 hover:text-pink-500 transition-colors z-10">
                     <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-12 h-12">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M9 15L3 9m0 0l6-6M3 9h12a6 6 0 010 12h-3" />
                    </svg>
                </button>
                <h2 className="text-5xl font-bold text-center text-purple-700 mb-4 mt-12">{selectedLesson.title}</h2>

                <div className="aspect-square w-full bg-gray-100 rounded-2xl overflow-hidden shadow-inner my-4 flex items-center justify-center">
                    <img src={card.imageUrl} alt={card.word} className="w-full h-full object-contain" />
                </div>
                
                <div className="text-center">
                    <div className="flex items-center justify-center gap-4">
                        <h3 className="text-8xl font-bold text-gray-800 capitalize">{card.word}</h3>
                        <ReadAloudButton text={card.word} isSoundOn={isSoundOn} lang="en" />
                    </div>
                    <p className="text-4xl text-pink-500 font-semibold mb-2">{card.vietnamese}</p>
                    <div className="flex items-center justify-center gap-2">
                         <p className="text-2xl text-gray-600">{card.sentence}</p>
                         <ReadAloudButton text={card.sentence} isSoundOn={isSoundOn} lang="en" />
                    </div>
                </div>
                
                <div className="flex justify-between items-center mt-6">
                    <button onClick={handlePrevCard} disabled={currentCard === 0} className="px-8 py-4 bg-gray-300 rounded-lg font-bold text-gray-600 disabled:opacity-50 text-xl">
                        Trước
                    </button>
                    <span className="font-semibold text-purple-700 text-xl">{currentCard + 1} / {lessonData.length}</span>
                    <button onClick={handleNextCard} className="px-8 py-4 bg-green-400 rounded-lg font-bold text-white hover:bg-green-500 text-xl">
