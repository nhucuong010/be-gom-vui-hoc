...
// This data is now embedded in the app via uiSounds.ts.
 ...
 * @param isSoundOn A boolean to conditionally play the sound.
 */
export const playDynamicSentence = (text: string, lang: 'vi' | 'en', isSoundOn: boolean, game?: 'restaurant' | 'street_food' | 'garden_memory' | 'bunny_rescue' | 'english_story' | 'capybara_rescue'): Promise<void> => {
  return new Promise((resolve) => {
    ...
    const fallbackToTTS = () => {
      ...
      console.warn(`Pre-generated audio not found or failed for "${filename}". Falling back to TTS.`);
      speakSentence(text, lang, isSoundOn).finally(resolve); // `resolve` is implicitly handled by speakSentence's promise chain
    };
    ...
    audio.play().catch(error => {
      console.error(`Could not play pre-generated file ${audioSrc}:`, error);
