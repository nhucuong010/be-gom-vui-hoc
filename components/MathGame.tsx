...
const MathGame: React.FC<MathGameProps> = ({ onGoHome, onCorrectAnswer, isSoundOn }) => {
    ...
    return (
        <div className="w-full max-w-4xl mx-auto flex flex-col items-center p-4">
            <div className="w-full bg-white bg-opacity-80 backdrop-blur-sm rounded-3xl shadow-xl p-6 md:p-8 relative">
                ...
                <h2 className="text-4xl sm:text-5xl font-bold text-center text-purple-700 mb-2 mt-12">Bé Học Toán</h2>
                <p className="text-center text-xl sm:text-2xl text-pink-500 mb-8 sm:mb-12 font-semibold">Con muốn học dạng toán nào?</p>
  ...
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col items-center p-4">
      ...
      `}</style>
      <div className="w-full bg-white bg-opacity-80 backdrop-blur-sm rounded-3xl shadow-xl p-4 sm:p-6 md:p-8 relative">
        ...
        <button onClick={() => { playSound('click', isSoundOn); stopAllSounds(); setGameType(null); }} className="absolute top-4 left-4 text-purple-500 hover:text-pink-500 transition-colors">
          <ArrowLeftIcon className="w-8 h-8 sm:w-10 sm:h-10" />
        </button>

        <h2 className="text-4xl sm:text-5xl font-bold text-center text-purple-700 mb-1 mt-10 sm:mt-12">Bé Học Toán</h2>
        <p className="text-center text-xl sm:text-2xl text-pink-500 mb-4 sm:mb-6 font-semibold">Cấp độ {level}</p>
        ...
        {problem && (
          <div className="text-center transition-opacity duration-500">
            <div className="mb-4 min-h-[120px] flex flex-col items-center justify-center p-2 sm:p-4 bg-purple-50 rounded-xl shadow-inner">
            ...
            <div className="grid grid-cols-3 gap-3 sm:gap-4 md:gap-6">
              {options.map(opt => (
                <button
                  ...
                  className="bg-sky-400 hover:bg-sky-500 text-white font-bold text-4xl sm:text-5xl md:text-6xl py-6 sm:py-8 rounded-2xl shadow-lg transform transition-transform duration-200 hover:scale-110 active:scale-95 disabled:opacity-50"
                  disabled={!!feedback}
                >
                  {String(opt)}
                </button>
